...
import wandb
wandb.login(key='f647d6cfd571487fcdb4fb230c927cd2d675252a')
...
sweep_id = wandb.sweep(sweep_config, project="summarization_no_trainer")
...
#        accelerator_log_kwargs["log_with"] = args.report_to
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers(project_name="summarization_no_trainer")
        if args.push_to_hub:
    ...
    opt_option = wandb.config.optimizer
    if opt_option == 'adam':
        optimizer = torch.optim.AdamW(optimizer_grouped_parameters, lr=args.learning_rate)
    elif opt_option == 'adafactor':
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            with torch.no_grad():
                ...
                # print('1. print(generated_tokens):', generated_tokens)
                generated_tokens = accelerator.pad_across_processes(
                    generated_tokens, dim=1, pad_index=tokenizer.pad_token_id
                )
                # print('2. print(generated_tokens):', generated_tokens)
                labels = batch["labels"]
                if not args.pad_to_max_length:
                    # If we did not pad to max length, we need to pad the labels too
                    labels = accelerator.pad_across_processes(batch["labels"], dim=1, pad_index=tokenizer.pad_token_id)
                generated_tokens, labels = accelerator.gather_for_metrics((generated_tokens, labels))
                # print('3. print(generated_tokens):', generated_tokens)
                generated_tokens = generated_tokens.cpu().numpy()
                # print('4. print(generated_tokens):', generated_tokens)
                ...
                # print('decoded_preds before postprocess: ', decoded_preds)
...
if __name__ == "__main__":
    wandb.agent('xtz21od6', function=main, count=11)
